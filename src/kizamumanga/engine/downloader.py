            ...
            for img_name, url in img_dict.items():
                async with aiohttp.ClientSession() as session:
                    if url.strip() == "" or url == "N/A":
                        self._logger.error(
                            f"Invalid URL for image {img_name} in chapter {chapter_url}"
                        )
                        raise MangaError(
                            f"Invalid URL for image {img_name} in chapter {chapter_url}"
                        )
                            ...
                            async with session.get(
                                ...
                                try:
                                    if not self.config.color:
                                        imgc.grayscale()
                                        self._logger.info(
                                            f"image color changed to grayscale at {img_path}"
                                        )

                                    if self.config.cropping_mode:
                                        is_grayscale = not self.config.color
                                        imgc.crop_countors(
                                            img_is_grayscale=is_grayscale
                                        )
                                        self._logger.info(
                                            f"image cropped at {img_path}"
                                        )

                                    if width is not None and height is not None:
                                        self._logger.info(
                                            f"image resized at {img_path}"
                                        )
                                        imgc.resize(width=width, height=height)

                                except Exception as e:
                                    self._logger.error(
                                        f"Processing failed for {img_path}: {e}"
                                    )
                                    raise
                                break
                        except asyncio.TimeoutError:
                            self._logger.error(
                                f"TimeoutError while downloading {img_name} from {url}"
                            )
                        except FileNotFoundError:
                            self._logger.error("Download interrupted by user")
            self._logger.info(
                f"Chapter downloaded successfully: {chapter_url} at {path}"
            )
            return True
